  ...
  return (
    <div className="max-w-4xl mx-auto space-y-6">
      <div className="flex items-center justify-between">
        <div><h1 className="text-3xl font-bold mb-1">Auto-Reply</h1><p className="text-gray-600 dark:text-gray-400 font-medium">Pre-written responses auto-queued when conditions are met.</p></div>
      ...
      <div className="card !border-green-500 bg-green-100 dark:bg-green-500/10">
        <div className="flex gap-3">
          <Zap size={18} className="text-green-700 dark:text-green-400 shrink-0 mt-0.5" />
          <div>
            <p className="text-sm font-bold text-green-800 dark:text-green-400 mb-1">How Auto-Reply Works</p>
            <p className="text-xs text-green-700 dark:text-gray-400">Create response templates with conditions. When a job matches your criteria, the reply is auto-queued and ready to send.</p>
          </div>
        </div>
      </div>
      ...
      {showForm && (
        <div className="card">
          ...
          <div className="space-y-4">
            <div><label className="text-xs font-bold text-gray-500 mb-1 block">Template Name</label><input className="input" placeholder="e.g. High-match quick response" value={form.name} onChange={e => setForm({...form, name: e.target.value})} /></div>
            <div><label className="text-xs font-bold text-gray-500 mb-1 block">Reply Message</label>
              <textarea className="input min-h-[150px] text-sm" placeholder="Hi! I noticed your project..." value={form.message} onChange={e => setForm({...form, message: e.target.value})} />
            </div>
            <div className="grid grid-cols-2 gap-4">
              <div><label className="text-xs font-bold text-gray-500 mb-1 block">Condition</label>
              ...
              <div><label className="text-xs font-bold text-gray-500 mb-1 block">Min Score Threshold</label>
      ...
      {autoReplyTemplates.length === 0 && !showForm ? (
        <div className="card text-center py-16">
          <MessageSquare size={40} className="mx-auto mb-4 text-gray-300 dark:text-gray-600" />
          <h3 className="font-bold mb-2">No Auto-Reply Templates</h3>
          <p className="text-sm text-gray-500 max-w-md mx-auto mb-4">Create templates that auto-queue when conditions are met.</p>
            ...
            <div key={t.id} className="card card-hover">
              <div className="flex items-center justify-between mb-3">
                <div className="flex items-center gap-3">
                  ...
                  <span className={`badge text-[10px] ${t.enabled ? "bg-green-200 text-green-800 dark:bg-green-500/20 dark:text-green-400" : "bg-gray-200 dark:bg-gray-500/20 text-gray-600 dark:text-gray-500"}`}>{t.enabled ? "Active" : "Paused"}</span>
                </div>
                <div className="flex items-center gap-2">
                  ...
                  <button onClick={() => updateAutoReplyTemplate(t.id, { enabled: !t.enabled })} className="btn-ghost !p-2">
                    {t.enabled ? <ToggleRight size={16} className="text-green-600" /> : <ToggleLeft size={16} />}
                  </button>
                  <button onClick={() => removeAutoReplyTemplate(t.id)} className="btn-ghost !p-2 text-red-500"><Trash2 size={16} /></button>
                </div>
              </div>
              <div className="text-xs font-bold text-gray-500">
                Condition: {t.condition === "score_above" ? `Score > ${t.minScore}%` : t.condition === "any_reply" ? "Any reply" : "Saved jobs"}
              </div>
              {preview === t.id && (
                <div className="mt-4 p-4 border-2 border-black/20 dark:border-green-500/20 bg-gray-50 dark:bg-white/5">
